...
def main(request):
    ...
    if request.user.is_authenticated:
        ...
        contact_list = Contact.objects.filter(user=request.user).order_by('-id')
...
def contact_book(request):
    ...
    if 'find_contact_criteria' in request.GET and 'find_contact_value' in request.GET:
        ...
        if search_form.is_valid():
            ...
            if search_criteria == 'name':
                contact_list = contact_list.filter(user=request.user, 
    ...
    upcoming_birthdays = get_upcoming_birthdays(request, 7)
    if 'days_ahead' in request.GET:
        ...
        if days_ahead_form.is_valid():
            ...
            contact_list = contact_list.filter(id__in=[contact.id for contact in get_upcoming_birthdays(request, days_ahead)]).order_by('-id')
...
def get_upcoming_birthdays(request, days_ahead):
    ...
    try:
        ...
        days_ahead = 7
    upcoming_birthdays = Contact.objects.get_birthdays_in_days(days_ahead).filter(user=request.user)
